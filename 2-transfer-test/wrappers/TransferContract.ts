...
export class TransferContract implements Contract {
    ...
    */
    ...
    async sendIncrementWithJetton(
        ...
        const msg = internal({
            ...
            body: beginCell()
                ...
                .storeAddress(via.address!)  // yanıt adresi (gönderen)
                .storeBit(0)                 // özel payload yok
                .storeCoins(toNano('0.01'))  // forward miktarı
                .storeBit(0)                 // forward payload yok
                .endCell()
        });
    
        await provider.internal(via, {
            value: toNano('0.1'),  // Toplam işlem değerini arttırdım
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().store(storeMessageRelaxed(msg)).endCell()
        });
    }
    ...
    async sendWithdraw(provider: ContractProvider, via: Sender) {
        const MIN_TON = toNano(0.1); // 0.01 TON for fees
        await provider.internal(via, {
            value: MIN_TON,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell()
